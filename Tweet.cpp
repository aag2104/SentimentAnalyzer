...
std::vector<DSString> Tweet::tokenize()
{
    ...
    do
    {
        ...
        if(message[index+1] == '\0'){ //if you are about to be at the end of the vector, you want to store current
           words.push_back(currWord); //store
           break; //end
        }
    ...
    for(int i = 0; i < words.size(); i++){ //for some reason i'm still storing blanks
        if(words[i] == "\0"){
